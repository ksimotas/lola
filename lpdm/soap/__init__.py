...
Credits:
    https://github.com/ClashLuke/SOAP
"""
...
class SOAP(torch.optim.Optimizer):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            p_list, grad, grad_projected, exp_avg, exp_avg_sq = zip(*vals, strict=True)
            ...
            for p, g, ea, d in zip(p_list, grad, exp_avg, denom, strict=True):
                state = self.state[p]
    ...
    def project(self, grad, state, merge_dims=False, max_precond_dim=10000):
        ...
        for mat in state["Q"]:
            if len(mat) > 0:
                ...
                permute_order = [*range(1, len(grad.shape)), 0]
                ...
                for idx, sh in enumerate(grad.shape):
                    if sh <= max_precond_dim:
                        outer_product = torch.tensordot(
                            ...
                            dims=[(*range(idx), *range(idx + 1, len(grad.shape)))] * 2,
                        )
                        state["GG"][idx].lerp_(outer_product, 1 - state["shampoo_beta"])
